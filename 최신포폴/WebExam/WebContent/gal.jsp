...
%>
<%
	ArrayList<G_BoardBean> articleList = (ArrayList<G_BoardBean>) request.getAttribute("articleList");
	...
	<section id="main">
		<section id="mainright">
			<div id="matList">
				...
				<a class="btn btn-default pull-right" href="galBoardWrite.jsp">글쓰기</a>
				...
				<div class="article_pic" style="font-size: 30pt; font-weight: bold;">
					...
					<table class="table table-striped">
						...
						<%
								for (int i = 0; i < articleList.size(); i=i+3) {
									
						%>			
									<tr>
										<td class="picli">
											<a href="boardDetail.bo?board_num=<%=articleList.get(i).getGB_NUM()%>&page=<%=nowPage%>"> <img src="boardUpload/<%=articleList.get(i).getGB_FILE()%>" class="imgst img-thumbnail">
											</a><br>
											<%=articleList.size() %><BR>
											<%=articleList.get(i).getGB_SUBJECT()%><br>
											<%=articleList.get(i).getGB_ID()%><br>
											<%=articleList.get(i).getGB_DATE()%>
											<%=articleList.get(i).getGB_READCOUNT()%>
										</td>
										<%if(i+1>=articleList.size())
											break; %>
										<td class="picli">
											<a href="boardDetail.bo?board_num=<%=articleList.get(i + 1).getGB_NUM()%>&page=<%=nowPage%>"> <img src="boardUpload/<%=articleList.get(i + 1).getGB_FILE()%>" class="imgst img-thumbnail">
											</a><br>
											<%=articleList.get(i + 1).getGB_SUBJECT()%><br>
											<%=articleList.get(i + 1).getGB_ID()%><br>
											<%=articleList.get(i + 1).getGB_DATE()%>
											<%=articleList.get(i + 1).getGB_READCOUNT()%>
										</td>
										<%if(i+2>=articleList.size())
											break; %>
										<td class="picli">
											<a href="boardDetail.bo?board_num=<%=articleList.get(i + 2).getGB_NUM()%>&page=<%=nowPage%>"> <img src="boardUpload/<%=articleList.get(i + 2).getGB_FILE()%>" class="imgst img-thumbnail">
											</a><br>
											<%=articleList.get(i + 2).getGB_SUBJECT()%><br>
											<%=articleList.get(i + 2).getGB_ID()%><br>
											<%=articleList.get(i + 2).getGB_DATE()%>
											<%=articleList.get(i + 2).getGB_READCOUNT()%>
										</td>
									</tr>
						<%
								}
